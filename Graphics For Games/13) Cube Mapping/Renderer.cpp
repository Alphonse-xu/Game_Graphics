...
void Renderer::DrawParticles()
{
	...
	glEnable(GL_BLEND);
	glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
	projMatrix = Matrix4::Perspective(1.0, 15000.0f, (float)width / (float)height, 45.0f);
	glUniform1i(glGetUniformLocation(currentShader->GetProgram(), "snowtexture"), 0);
	textureMatrix.ToIdentity();
	for (int i = 0; i < NUM_OF_PARTICLES; ++i) {
		modelMatrix = Matrix4::Translation(ParticleSystem[i]->Getposition()) * Matrix4::Scale(ParticleSystem[i]->GetScale());
		modelMatrix.Transpose(viewMatrix);
		UpdateShaderMatrices();
		ParticleSystem[i]->shape->SetTexture(snowTexture);
